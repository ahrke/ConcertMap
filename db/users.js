...
const getUserByEmail = async(email) => {
  const res = db.query(`SELECT usr.id, usr.email, usr.password FROM users usr WHERE email = $1;`, [email]);
...
const insertUser = (user) => {
  const res = db.query('INSERT INTO users (email, password) VALUES ($1, $2) RETURNING users.id', [user['email'], user['password']]);
  if (res.rowCount === 0) throw new Error('User creation failed');
...
const upsertUserProfile = async(profile) => {
  const query = `
    ...
    RETURNING user_id;
  `;
  const res = await db.query(query, [profile['user_id'], profile['avatar_uri'], profile['bio']]);
  if (res.rowCount === 0) throw new Error('Update profile failed');
};
